...
/*
template <class T> T Lista<T>::buscarVariable(Lista* var, string cadena){ // es el codigo de la clase practica puesto como funcion
    int error=0;
    Pila *p=new Pila();
    string pf;
for(int i=0;(i<cadena.length())&&(!error);i++){
    ...
    if((!error)&&p->pilavacia())cout<<endl<<"Todo OK, expresión balanceada"<<endl;
...
//conversion de entrefijo a posfijo
      ...
      {   
      	...
      	//extraeNombreVariable(cadena); //obtengo el nombre de la variable de la expresion
		if( (d>='a') && (d<='z') ){ // si se encuentra una variable en la expresion
			int a = var->ExtraeValorEntero(d);
			stringstream ss;
			ss << a;
			string str = ss.str();
...
template <class T> class Lista{
    ...
    public:
            ...
            T EvaluaExpresion(Lista lis, string c);
            T EvaluaBooleano(Lista lis, string c);
            bool esvacia(void);
            T cabeza(void);
            //char cabezavar(void);
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------
//    FUNCIONES EVALUDAORAS

// REVISAR, DA EROOR --> [Error] prototype for 'T Lista<T>::EvaluaExpresion(Lista<T>*, std::string)' does not match any in class 'Lista<T>'

template <class T> T Lista<T>::EvaluaExpresion(Lista* var, string cadena){
    int error=0;
    Pila *p=new Pila();
    string pf;
for(int i=0;(i<cadena.length())&&(!error);i++){
    ...
    if((!error)&&p->pilavacia())cout<<endl<<"Todo OK, expresión aritmética balanceada"<<endl;
    else cout<<endl<<"ERROR";
    cout<<"\n Inicio conversion a POSFIJO de: "<<cadena<<endl;
//conversion de entrefijo a posfijo
      char d,p1;
...
template <class T> T Lista<T>::EvaluaBooleano(Lista* var, string cadena){
    int error=0;
    Pila *p=new Pila();
    string pf;
for(int i=0;(i<cadena.length())&&(!error);i++){
    ...
    if((!error)&&p->pilavacia())cout<<endl<<"Todo OK, expresión booleana balanceada"<<endl;
    else cout<<endl<<"ERROR";
    cout<<"\n Inicio conversion a POSFIJO de:"<<cadena<<endl;
	//conversion de entrefijo a posfijo
    char d,p1;
...
// FIN FUNCIONES EVALUADORAS
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------



//-----------------------------------------------------------------------------------------------------------------------------------------------------------------
//INICIO DEFINICIÓN DE CLASES
...
class Asignacion:public Instruccion{
	...
	public:
		// Constructores
		Asignacion(){this->linea = "";};
		Asignacion(string linea){this->linea = linea;};
		~Asignacion();
		//Metodos
		string getAsignacion(){return this->linea;}
		void ejecutar(Lista<Variable> *variables, Lista<Instruccion> *instrucciones, int i){
...
class Condicional:public Instruccion{
	private:
		string condicional;
		string getCondicional(){return this->condicional;}
	public:
		Condicional(){this->condicional="";};
		Condicional(string c){this->condicional = c;};
		void ejecutar(Lista<Variable> *variables, Lista<Instruccion> *instrucciones, int i){
			int inicio = 0, fin=0;
			string c,f;
			for(int j=0; j<instrucciones->length(); j++){
                if(instrucciones->at(j) == '(') inicio = j;
                if(instrucciones->at(j) == ')') fin = j;
                }
                    for(int k = inicio;k<=fin;k++)
                        c.push_back(instrucciones->at(k));
                    f = instrucciones->substr(instrucciones->find("THEN")+4);
                    if(EvaluaBooleano(var,c)){
                      switch(buscarPalabra(f)){
                            ...
                            case 4:// si hay que hacer un jump
            						...
            						while(i<aux){instrucciones = instrucciones->resto(); i++; } // mientras no se llegue a la línea que pide en el JUMP, itera los nodos
                                    break;
                            case 5: // si hay que mostrar alguna expresion aritmetica
                                    cout << endl << EvaluaExpresion(var,instrucciones->substr(instrucciones->find("SHOW") +1 )) << endl;
                                    instrucciones = instrucciones->resto();
                                    break;
                            }
                    }
                    else{instrucciones = instrucciones->resto();}
                    break;
...
class Salto:public Instruccion{
	...
	public:
		Salto(){this->salto ="";};
		Salto(string jump){this->salto = jump;};
		string getSalto(){return this->salto;};
...
class Show:public Instruccion{
	...
	public:
		Show(){this->mostrar = "";};
		Show(string show){this->mostrar = show;};
		string getMostrar(){return this->mostrar;};
		void ejecutar(Lista<Variable> *variables, Lista<Instruccion> *instrucciones, int i){
		...
		*comando = instrucciones->cabeza();
		switch(search(comando->getInstruccion())){
			...
			case 3: {Asignacion *a = new Asignacion(comando->getInstruccion());
					...
					instrucciones = instrucciones->resto();
					break;}
			case 4:{Salto *s = new Salto(comando->getInstruccion());
